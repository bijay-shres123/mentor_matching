...
from profiles_api.models import Student, Mentor, UserProfile, Preference, Notification
...
def create_matches():
    ...
    for grp1, grp2 in zip(result[0], result[1]):
        first_user = Student.objects.get(id=int(grp1))
        second_user = Mentor.objects.get(id=int(grp2))
        
        first_user.mentor_or_mentee = second_user.user
        second_user.mentor_or_mentee = first_user.user

        first_message = f"{second_user.full_name} ({second_user.user.email})has been assigned as your mentor."
        second_message = f"{first_user.full_name} ({first_user.user.email})has been selected as your student."

        Notification.objects.create(user=first_user.user, message=first_message)
        Notification.objects.create(user=second_user.user, message=second_message)        
